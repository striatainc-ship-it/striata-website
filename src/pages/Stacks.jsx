...
function SafetyBadge({ safety }) {
  if (safety === 'SAFE') {
    return (
      <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-wide bg-emerald-500/15 text-emerald-400 border border-emerald-500/25">
        {SHIELD_SVG} Safe
      </span>
    )
  }
  return (
    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-wide bg-amber-500/15 text-amber-400 border border-amber-500/25">
      {WARN_SVG} Caution
    </span>
  )
}
...
function StackCard({ stack }) {
  ...
  return (
    <article
      className="rounded-2xl overflow-hidden border border-white/8 bg-[#0d1e35] flex flex-col"
    ...
    >
      ...
      <div className="border-b border-white/8">
        <div className="grid grid-cols-[1fr_2.5fr_1fr] bg-[#0A1628] px-4 py-2">
          <span className="text-white/40 text-xs font-semibold uppercase tracking-widest">Tier</span>
          <span className="text-white/40 text-xs font-semibold uppercase tracking-widest">Includes</span>
          <span className="text-white/40 text-xs font-semibold uppercase tracking-widest text-right">Price</span>
        </div>

        {stack.tiers.map((tier) => {
          const isEntry   = tier.label === 'Entry'
          const isPremium = tier.label === 'Premium'

          const rowBg    = isPremium ? '#0B3D2B'        : isEntry ? meta.lightColor : 'rgba(232,237,242,0.06)'
          const labelCol = isPremium ? '#4ADE80'        : meta.color
          const priceCol = isPremium ? '#4ADE80'        : meta.color
          const textCol  = isPremium ? 'rgba(255,255,255,0.85)' : 'rgba(255,255,255,0.65)'
          ...
          return (
            ...
            >
              <span className="text-xs font-bold uppercase tracking-wide" style={{ color: labelCol }}>
                {tier.label}
              </span>
              <span className="text-xs leading-snug pr-2" style={{ color: textCol }}>
                {tier.products}
              </span>
              <span className="text-xs font-mono font-bold text-right" style={{ color: priceCol }}>
                {tier.price}
              </span>
            </div>
          )
      ...
      >
        <p className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: stack.safety === 'SAFE' ? '#4ADE80' : '#FCD34D' }}>
          Safety Note
        </p>
      ...
      {stack.advisory && (
        <div className="px-5 py-4 border-b border-white/8" style={{ backgroundColor: 'rgba(239,68,68,0.05)', borderLeft: '3px solid rgba(239,68,68,0.35)' }}>
          <p className="text-xs font-bold uppercase tracking-widest text-red-400 mb-1">Usage Advisory</p>
      ...
      <div className="border-b border-white/8">
        <div className="px-5 py-2.5 bg-[#0A1628]">
          <p className="text-xs font-bold uppercase tracking-widest text-white/40">Contraindications — Do Not Use If:</p>
        </div>
      ...
      <div className="p-4 mt-auto">
        <a
          ...
          className="flex items-center justify-center gap-2.5 w-full py-3.5 rounded-xl font-bold text-sm text-white transition-all duration-200 hover:brightness-110 hover:shadow-lg cursor-pointer"
          style={{ backgroundColor: '#00B4B4', boxShadow: 'none' }}
...
function UniversalContraindications() {
  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-white/8 bg-[#0d1e35]">
      ...
      >
        <div>
          <p className="text-xs font-bold uppercase tracking-widest text-white/40 mb-0.5">Applies to All Stacks</p>
    ...
    <div className="bg-[#0A1628] min-h-screen">
      ...
      <section className="relative pt-36 pb-20 px-6 overflow-hidden">
        ...
        <div
          ...
          style={{
            backgroundImage: 'radial-gradient(circle at 20% 50%, #00B4B4 0%, transparent 50%), radial-gradient(circle at 80% 20%, #8B5CF6 0%, transparent 40%)',
          }}
        />
            ...
            {FILTERS.map(({ id, label }) => {
              ...
              return (
                <button
                  ...
                  className={`flex-none px-4 py-2 rounded-full text-xs md:text-sm font-semibold whitespace-nowrap transition-all duration-200 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#00B4B4] ${
                    isActive
                      ? 'text-[#0A1628] shadow-lg'
                      : 'bg-[#0d1e35] border border-white/10 text-white/60 hover:text-white hover:border-white/20'
                  }`}
                  style={isActive ? { backgroundColor: meta ? meta.color : '#00B4B4' } : {}}
                >
                  {label} <span className={isActive ? 'opacity-70' : 'opacity-40'}>({count})</span>
                </button>
              )
            })}
          </div>
