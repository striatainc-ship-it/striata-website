...
function renderBlock(block, key) {
  ...
  // Blockquote callout — every line starts with ">"
  if (lines.every(l => l.trim().startsWith('>'))) {
    ...
    return (
      <div
        key={key}
        className="my-6 rounded-xl border border-[#00B4B4]/25 bg-[#00B4B4]/[0.06] px-5 py-4 border-l-4 border-l-[#00B4B4]"
      >
        {inner.map((p, j) => (
          <p key={j} className={`text-white/75 text-[15px] leading-relaxed ${j > 0 ? 'mt-3' : ''}`}>
            {p.split('\n').map((ln, k) => (
              <span key={k}>
                {renderInline(ln)}
                {k < p.split('\n').length - 1 && <br />}
  ...
  if (first.startsWith('### ')) {
    return (
      <h3 key={key} className="text-base md:text-lg font-bold text-[#00B4B4] mt-9 mb-3 uppercase tracking-wide" style={{ fontFamily: 'Montserrat, sans-serif' }}>
  ...
  if (first.startsWith('#### ')) {
    return (
      <h4 key={key} className="text-base md:text-lg font-bold text-white mt-7 mb-2" style={{ fontFamily: 'Montserrat, sans-serif' }}>
        {renderInline(first.slice(5))}
      </h4>
    )
  }
        ...
        <div key={key} className="my-6 overflow-x-auto">
          <table className="w-full text-sm border border-white/8 rounded-xl overflow-hidden">
            <thead className="bg-[#0d1e35]">
              <tr>
                {headers.map((h, j) => (
                  <th key={j} className="text-[#00B4B4] text-left py-3 px-4 font-semibold border-b border-white/8 whitespace-nowrap" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                    {renderInline(h)}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {rows.map((row, j) => (
                <tr key={j} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                  {row.map((cell, k) => (
                    <td key={k} className="text-white/70 py-2.5 px-4 align-top">
                      {renderInline(cell)}
                    </td>
                  ))}
                </tr>
  ...
  if (bulletLines.length > 0 && bulletLines.every(l => l.trim().startsWith('- '))) {
    return (
  ...
  // Standalone italic line — used as a subtitle under a heading, or the closing disclaimer
  if (lines.length === 1 && trimmed.startsWith('*') && trimmed.endsWith('*') && !trimmed.startsWith('**')) {
    return (
      <p key={key} className="text-white/45 text-sm italic leading-relaxed -mt-1 mb-5">
        {trimmed.slice(1, -1)}
      </p>
    )
  }
    ...
    return (
      <div className="bg-[#0A1628] min-h-screen flex items-center justify-center px-6">
        <div className="text-center">
          ...
          <Link to="/guides" className="text-[#00B4B4] hover:text-white transition-colors">
            ← Back to Guides
          </Link>
        </div>
      </div>
      ...
      <section className="py-20 border-t border-white/5 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-2xl mx-auto text-center px-6">
          ...
          <p className="text-white/60 mb-8">
            Not sure about a dose, a reconstitution volume, or which markers to test? Message us — we'd rather answer a question than have you guess.
          </p>
          <a
            href={whatsappLink}
            target="_blank"
